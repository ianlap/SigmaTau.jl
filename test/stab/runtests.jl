...
@testset "SigmaTau Math Core Parity" begin
    ...
    @testset "Legacy parity (extracted SP1065 kernels)" begin
        ...
        for m in m_grid
            ...
            @test SigmaTau._adev_core(x, [m], tau0)[1] ≈ new_dev atol=at rtol=rt
        end

        # MDEV.
        for m in m_grid
            new_dev = sqrt(LK.mdev_var(x, m, tau0, x_cs))
            @test SigmaTau._mdev_core(x, [m], tau0)[1] ≈ new_dev atol=at rtol=rt
        end

        # HDEV.
        for m in m_grid
            new_dev = sqrt(LK.hdev_var(x, m, tau0))
            @test SigmaTau._hdev_core(x, [m], tau0)[1] ≈ new_dev atol=at rtol=rt
        end

        # MHDEV.
        for m in m_grid
            new_dev = sqrt(LK.mhdev_var(x, m, tau0, x_cs))
            @test SigmaTau._mhdev_core(x, [m], tau0)[1] ≈ new_dev atol=at rtol=rt
        end
        ...
        for m in [1, 2, 4, 8, 16]
            ...
            @test SigmaTau._mtotdev_core(x, [m], tau0)[1] ≈ new_dev atol=at rtol=rt
        end
    ...
    @testset "Stable32 cross-validation (reference/validation/)" begin
        ...
        if !isfile(dat_path) || !isfile(csv_path)
            ...
            for row in rows
                ...
                # Bonus: the new SigmaTau core matches the reference
    ...
    @testset "MTOTDEV across all 5 power-law noise types" begin
        ...
        for (alpha, label) in cases
            ...
            for m in ms
                ...
                got = SigmaTau._mtotdev_core(x, [m], tau0)[1]
    ...
    @testset "ADEV/HDEV across all 5 power-law noise types" begin
        ...
        for alpha in (2.0, 1.0, 0.0, -1.0, -2.0)
            ...
            x_cs = pushfirst!(cumsum(x), 0.0)
            for m in ms
                @test SigmaTau._adev_core(x, [m], tau0)[1]  ≈
                      sqrt(LK.adev_var(x, m, tau0))                 atol=1e-25 rtol=rt
                @test SigmaTau._mdev_core(x, [m], tau0)[1]  ≈
                      sqrt(LK.mdev_var(x, m, tau0, x_cs))            atol=1e-25 rtol=rt
                @test SigmaTau._hdev_core(x, [m], tau0)[1]  ≈
                      sqrt(LK.hdev_var(x, m, tau0))                  atol=1e-25 rtol=rt
                @test SigmaTau._mhdev_core(x, [m], tau0)[1] ≈
    ...
    @testset "TOTDEV/HTOTDEV EDF fallback for WPM/FLPM" begin
        ...
        edfs = SigmaTau.calculate_edf(:totdev, ones(length(m_values_eq)),
        ...
        edfs_h = SigmaTau.calculate_edf(:htotdev, ones(length(m_values_eq)),
                                                 noises_wpm, m_values_eq, taus_eq, N_eq, T_eq)
    ...
    @testset "MTIE core" begin
        ...
        devs = SigmaTau._mtie_core(x, [1, 2, 3, 4], 1.0)
        ...
        m_grid = [1, 2, 5, 10, 50]
        ramp_devs = SigmaTau._mtie_core(ramp, m_grid, 1.0)
        @test ramp_devs ≈ Float64.(m_grid)

        # Constant phase → MTIE = 0 at every τ.
        const_devs = SigmaTau._mtie_core(fill(3.14, 64), [1, 2, 4, 8], 1.0)
        @test all(==(0.0), const_devs)

        # NaN guard for windows wider than the record.
        @test isnan(SigmaTau._mtie_core(ramp, [200], 1.0)[1])
    ...
    @testset "PDEV core" begin
        ...
        Random.seed!(20260509)
        x = cumsum(randn(512))
        @test SigmaTau._pdev_core(x, [1], 1.0)[1] ≈
              SigmaTau._adev_core(x, [1], 1.0)[1] atol=0.0 rtol=1e-15

        # Constant phase → PDEV = 0.
        const_devs = SigmaTau._pdev_core(fill(2.718, 128), [1, 2, 4, 8], 1.0)
        @test all(==(0.0), const_devs)
        ...
        function _pdev_reference(x, m_values, tau0)
            ...
            for (idx, m) in enumerate(m_values)
                if m == 1
                    out[idx] = SigmaTau._adev_core(x, [1], tau0)[1]
                    continue
                end
                M = N - 2m
                if M < 1
    ...
    @testset "Zero-arg convenience: default octave m-grid per deviation" begin
        ...
        @testset "_default_m_values argument validation" begin
            @test_throws ArgumentError SigmaTau._default_m_values(1024, :nope)
            # N too small to admit any m ≥ 1
            @test_throws ArgumentError SigmaTau._default_m_values(1, :adev)
            @test_throws ArgumentError SigmaTau._default_m_values(3, :mhdev)
        end
        ...
        @testset "HTOTDEV at N=1536 stays clear of the diff(x) m-max" begin
            ...
            ms = SigmaTau._default_m_values(1536, :htotdev)
    ...
    @testset "_phase_to_freq / _freq_to_phase helpers" begin
        ...
        @testset "round-trip identities (modulo the lost offset/sample)" begin
            ...
            fd2 = SigmaTau._phase_to_freq(SigmaTau._freq_to_phase(fd))
            @test length(fd2.y) == length(y) - 1
            @test fd2.y ≈ y[2:end]
            # Phase → frequency → phase recovers x[2:end] − x[1].
            x   = cumsum(randn(64))
            pd  = PhaseData(x, τ₀)
            pd2 = SigmaTau._freq_to_phase(SigmaTau._phase_to_freq(pd))
        ...
        @testset "deviation equivalence: adev/mdev/hdev agree on either domain" begin
            ...
            f_diff = SigmaTau._phase_to_freq(p_full)
