...
- **Stable32 composite-noise cross-validation fixture** at
  `reference/validation/s32_5_12_26/`. N=32768 phase/frequency record
  generated by Stable32's NoiseGen with `σ(τ=1)` chosen to give each
  power-law noise type its own decade of τ (WPM/FPM 3e-10, WFM 1e-10,
  FFM 1e-11, RWFM 3e-13; FPM→WFM→FFM→RWFM crossovers at τ ≈ 10, 100,
  1000). Stable32 captures: multi-deviation stability run dump
  (`output_notfiltered.txt`) and single-τ sigma-function detail dump
  (`s32_sinlgetau`, with EDF / Chi-Square-DF / B1 / noise-type / CI).
  Tooling: `process.py` parses Stable32 dumps to long-format CSV;
  `compute_sigmatau.jl` runs the eight SigmaTau deviations at the same
  AF ladder and emits `sigmatau_deviations.csv`; `compare.py` joins
  both and writes a row-by-row `relative_diffs.csv` plus optional
  overlay plots (regenerable, gitignored). Current parity vs Stable32:
  σ matches to 5 sig figs across all 8 deviations (max |Δ| 4.3e-5);
  σ_min / σ_max match to 3–4 sig figs (max |Δ| 1.3e-3). The README
  documents known Stable32 quirks (`Hadamard Dev=0` UI bug in the
  single-τ dialog, omitted TTOTDEV / HTOTDEV deviation fields, long-τ
  B1 noise-ID instability, MTOT-bias documentation contradiction).
...
- **Bias correction for `totdev` / `mtotdev` / `htotdev` was wrong in
  ...
  the SP1065 unbias is applied as `σ ← σ/√B`, the `:htot` table
  matches Howe 2005 Table I exactly, and α coverage extended to
  {−4, …, 0}. The `bias_correction` docstring was rewritten to make
  the variance-scale convention explicit, cite SP1065 / FCS 2001 /
  Howe & Vernotte 2003, and document the suspected typo in Riley's
  *"Confidence Intervals and Bias Corrections for the Stable32
  Variance Functions"* MTOT bias table (values appear to be `2 − B`
  of SP1065 / Greenhall 1999 and disagree on the direction of the
  bias; Stable32 empirically does not apply the correction it claims
  to either, so `correct_bias=false` matches Stable32's `mtotdev`).
- **`_coeff_htot` EDF table was wrong by ~20% at multiple α.** Values
  swapped to the canonical FCS 2001 / Howe & Tasset 2005 Table I
  `(b₀, b₁)` pairs. After the fix, the HTOTDEV EDF formula
  `(T/τ) / (b₀ + b₁·τ/T)` reproduces Stable32's reported EDF to
  better than 0.01 % for every AF in the paper's stated validity
  range `τ ≥ 16τ₀`. Closes the long-standing "HTOTDEV EDF off-by-one
  suspected" item from `R-MED-6` / the theory-pages spec.
- **`_coeff_mtot` EDF coefficients refit against Stable32 output.**
  The SP1065 / Stable32 manual values produce EDFs ~5–20 % below what
  ...
  documented inline as interim. Tracked in `TODO.md` for verification
  with additional Stable32 EDF dumps.
- **B1 noise-ID fallback (`_noise_id_b1rn`) was using the wrong `N`.**
  The `N` argument to `_b1_theory` was `length(x_dec) − 2`
  (post-decimation count), which collapses the dynamic range of the
  theoretical B1 values at small `N_eff` and misclassifies long-τ
  points as steeper noise than they are. Per Howe 2005 *"Enhancements
  to GPS Operation… Using a Total Hadamard Deviation"* "for a data run
  of length N", `N` is the input record's frequency-sample count
  (`length(x) − 1`). Independently, the variance of m-averaged
  frequencies was computed with the biased `N`-divisor estimator;
  Barnes / Howe B1 theory assumes the unbiased `N−1`-divisor (Bessel)
  variance. Both fixes align SigmaTau's B1 noise-ID to Stable32
  empirically: at small N_eff (AF=2000, 4000, 10000 on a 32k record)
  classifications now match Stable32 exactly where they previously
  drifted into FFM/FPM.
