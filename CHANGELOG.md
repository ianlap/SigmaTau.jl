...
- **Removed the `Est` submodule.** Clock state-space (`AbstractClockModel`,
  `TwoStateClock`, `ThreeStateClock`), Kalman filter (`KalmanFilter`,
  `predict!`, `update!`, `prop!`), and PID steering (`PIDController`,
  `step!`, `steer_to_correction`) moved to the sister package
  [ClockEnsemble.jl](https://github.com/ianlap/ClockEnsemble.jl).
  Callers must switch `using SigmaTau` → `using ClockEnsemble` for
  every state-space symbol.
- **Flattened the `Stab` submodule into the umbrella.** No more
  `SigmaTau.Stab.adev`; the unqualified `using SigmaTau; adev(pd, …)`
  form is unchanged because every Stab export was already re-exported
  at the umbrella level. Code that wrote the fully-qualified
  `SigmaTau.Stab.foo` or `SigmaTau.Est.foo` form will not resolve.
  `DEFAULT_CONFIDENCE` is now a top-level const in `SigmaTau` instead
  of `SigmaTau.Stab.DEFAULT_CONFIDENCE`.
- **Dropped `LinearAlgebra` and `Reexport` from `[deps]`** — neither
  is needed once the `Est` submodule and the `@reexport` macros are
  gone. `StaticArrays` stays in `[deps]` because the noise-ID
  quadratic-detrend hot path in `src/stab/noise/lag1.jl` uses
  `@SMatrix`/`@SVector` for its 3×3 LS solve.

### Removed

- Est-side exports: `AbstractClockModel`, `TwoStateClock`,
  `ThreeStateClock`, `KalmanFilter`, `PIDController`, `predict!`,
  `update!`, `prop!`, `step!`, `steer_to_correction`,
  `state_transition`, `process_noise`, `measurement_matrix`,
  `measurement_noise`, `nstates`. All available in
  [ClockEnsemble.jl](https://github.com/ianlap/ClockEnsemble.jl) v0.1.0.
- `src/est/`, `test/est/`, `examples/03_kalman_single_clock.jl`,
  `examples/04_kalman_pid_steering.jl`,
  `examples/05_holdover_comparison.jl`, the matching tutorials, and
  the `kalman.md`, `steering.md`, `ensemble_overview.md` theory pages.
  `docs/src/reference/est.md` removed.
