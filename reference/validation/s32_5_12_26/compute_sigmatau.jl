...
println("\nComputing deviations (ci=true, confidence=0.683)...")

t0 = time()
results = Dict{String,Any}()
results["adev"]    = adev(pd,    AF_SHORT; ci=true)
results["mdev"]    = mdev(pd,    AF_SHORT; ci=true)
results["tdev"]    = tdev(pd,    AF_SHORT; ci=true)
results["hdev"]    = hdev(pd,    AF_SHORT; ci=true)
results["totdev"]  = totdev(pd,  AF_LONG;  ci=true, correct_bias=true)
results["mtotdev"] = mtotdev(pd, AF_LONG;  ci=true, correct_bias=false)
results["ttotdev"] = ttotdev(pd, AF_LONG;  ci=true, correct_bias=false)
results["htotdev"] = htotdev(pd, AF_LONG;  ci=true, correct_bias=true)
