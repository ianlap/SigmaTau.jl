...
> **Audit date**: 2026-05-18 (post-v0.2.0 cut).
...
- [ ] **More `examples/`** — Literate pipeline currently ships
  ...
  - UDE drift learning via `DiffEqFlux.jl` — show how a `TwoStateClock`'s
    `predict!` composes with a NeuralODE that learns the unmodelled drift
    residual. Don't take SciML as a dep; keep it confined to the example.
...
- [ ] **Compat upper bounds** in the root `Project.toml`. The merged
  ...
  for AbstractFFTs, DocStringExtensions, RecipesBase, Reexport,
  StaticArrays, julia. Tighten upper bounds for `Distributions` and
  `StaticArrays` once the dep matrix has been exercised on the General
  registry.
...
See [CHANGELOG.md](CHANGELOG.md) for the annotated `## [0.2.0]` block.
